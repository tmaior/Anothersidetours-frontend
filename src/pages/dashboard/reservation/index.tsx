...
function Dashboard() {
    ...
    const fetchReservations = async () => {
      ...
      try {
          ...
          data.map(async (reservation) => {
            if (reservation.user_id) {
              if (!userDetails[reservation.user_id]) {
                ...
                return {
                  ...reservation,
                  user: userDetails[reservation.user_id],
                };
              }
            }
            return reservation;
        ...
        const groupedReservations: Record<string, Reservation> =
          reservationsWithUserDetails.reduce((acc, reservation) => {
            ...
            acc[date].reservations.push({
              ...
              time: new Date(reservation.reservation_date).toLocaleTimeString(
                "en-US",
                {
                  hour: "2-digit",
                  minute: "2-digit",
              ...
              statusColor:
                reservation.status === "PENDING" ? "red.500" : "green.500",
  ...
  const getNextDateWithReservations = () => {
    ...
    const nextDateIndex =
      reservations.findIndex((res) => res.date === lastLoadedDate) + 1;
    ...
    <DashboardLayout>
      ...
      >
        <Box
          ...
          borderRight={{
            base: "none",
            md: isDetailVisible ? "1px solid #e2e8f0" : "none",
          }}
              ...
              {filteredReservations.map((data, index) => (
                <ReservationItem
                  ...
                  onNoteClick={(notes, reservationId) =>
                    openNoteModal({
                      notes,
                      reservation: data.reservations.find(

...
import CustomLineItemsModal, {
  LineItem,
} from "../../../components/CustomLineItemsModal";
...
const GroupCard: React.FC<GroupCardProps> = ({
  ...
  const totalGuests = items.reduce(
    (sum, item) => sum + (item.guestQuantity || 0),
    0
  );
  ...
  const groupImage =
    items[0]?.tour?.imageUrl || "https://via.placeholder.com/50";
        ...
        >
          {items.map((item) => (
            <GuestItem
              ...
              avatarUrl={
                item.tour?.imageUrl || "https://via.placeholder.com/50"
              }
...
const PurchaseList = ({
  ...
  const filteredDisplayedReservations = displayedReservations.filter(
    (reservation) => {
  ...
  const { grouped, ungrouped } = groupReservations(
    filteredDisplayedReservations
  );
...
const PurchaseDetails = ({ reservation }) => {
  ...
  const [isChangeGuestQuantityModalOpen, setChangeGuestQuantityModalOpen] =
    useState(false);
  ...
  return (
    <VStack>
      <Box
        position="relative"
        w={{ base: "100%", md: "70%", xl: "100%", "2xl": "175%" }}
      ...
      >
        <Image
          src={
            reservation.tour.imageUrl || "https://via.placeholder.com/1000x300"
          }
...
const PaymentSummary = ({ reservation }) => {
  ...
  const [isChangeGuestQuantityModalOpen, setChangeGuestQuantityModalOpen] =
    useState(false);
  const [guestCount, setGuestCount] = useState(reservation?.guestQuantity || 0);
  const [isCustomLineItemsModalOpen, setIsCustomLineItemsModalOpen] =
    useState(false);
  ...
  useEffect(() => {
    const fetchAddons = async () => {
      if (!reservation?.id || !reservation?.tourId || !reservation?.tour?.id)
        return;
  ...
  const combinedAddons =
    reservation?.reservationAddons?.map((selectedAddon) => {
  ...
  const handleSaveCustomLineItems = async (items: LineItem[]) => {
    ...
    try {
      ...
      if (items.length > 0) {
        ...
        await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/custom-items`, {
  ...
  const addonsTotalPrice =
    combinedAddons && combinedAddons.length > 0
  ...
  const tourPrice =
    (reservation?.valuePerGuest || reservation?.tour?.price || 0) *
    (reservation?.guestQuantity || 0);
    ...
    >
      ...
      <VStack spacing={4} align="stretch" mt={4}>
        <HStack justifyContent="space-between">
          <Text>
            Guests (${reservation.tour?.price || 0} x{" "}
            {reservation.guestQuantity || 0})
          </Text>
          <Text>
            $
            ...
            (addon) =>
              addon && (
                <HStack key={addon.id} justifyContent="space-between">
                  <Text>
                    {addon.label || "Unnamed Add-on"} (${addon.price || 0} x{" "}
                    {addon.quantity || 0})
                  </Text>
                  <Text>
                    ${((addon.price || 0) * (addon.quantity || 0)).toFixed(2)}
                  </Text>
                </HStack>
              )
          )
            ...
            (item) =>
              item && (
                <HStack key={item.id} justifyContent="space-between">
                  <Text>
                    {item.name || "Unnamed"} (${item.amount || 0} x{" "}
                    {item.quantity || 0})
        ...
        <Box mt={8}>
          ...
          <Flex justify="space-between" mt={4} align="center">
            <Box>
              ...
              >
                <Text fontWeight="medium">Apply Code</Text>
              </Button>
            </Box>
          </Flex>
        </Box>
        ...
        purchaseSummary={{
          items: [
            ...
            ...(customLineItems?.length > 0
              ...
              : []),
            {
              name: `Guests (${reservation.tour?.price || 0} x ${
                reservation.guestQuantity || 0
              })`,
              price:
                (reservation.valuePerGuest || reservation.tour?.price || 0) *
...
const PurchasesPage = () => {
  ...
  const fetchAddonsForReservation = useCallback(
    async (reservationId: string, tourId?: string) => {
      try {
        ...
        return reservationAddons.map(
          (selectedAddon: { addonId: string; value: number }) => {
            const addonDetails = allAddons.find(
              (addon: { id: string; price: number; label: string }) =>
                addon.id === selectedAddon.addonId
            );
            return {
              name: addonDetails?.label || "Add-on",
        ...
        console.error(
          `Error fetching add-ons for reservation ${reservationId}:`,
          error
        );
        return [];
      }
    ...
    async (reservation: ReservationItem) => {
      ...
      if (reservation) {
        const formattedDate = new Date(
          reservation.createdAt || reservation.reservation_date
        ...
        try {
          if (reservation.isGroupBooking && reservation.groupItems) {
            const groupToursPromises = reservation.groupItems.map(
              async (item) => {
                const tourPrice = Number(
                  item.valuePerGuest || item.tour?.price || 0
                );
                const guestCount = Number(item.guestQuantity || 0);
                const gratuityAmount = Number(item.gratuityAmount || 0);
                const addons = await fetchAddonsForReservation(
                  item.id,
                  item.tour?.id
                );
                ...
                const tourTotal =
                  tourPrice * guestCount + gratuityAmount + addonsTotal;
            ...
            const totalAmount = groupTours.reduce(
              (sum, tour) => sum + tour.total,
              0
            );
    ...
    <Box width="100vw" height="100vh" overflow="hidden">
      <DashboardLayout>
        ...
        >
          ...
          >
            <Box padding="20px" marginBottom="20px" maxWidth="800px" mx="auto">
